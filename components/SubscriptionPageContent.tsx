...
export default function SubscriptionPageContent({ cars }: SubscriptionPageContentProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { t } = useI18n();
  const [filterType, setFilterType] = useState<FilterType>('suscripcion');
  const [suscripcionType, setSuscripcionType] = useState<SuscripcionType>('larga');
  const maxCarPrice = Math.max(...cars.map(car => car.pricePerMonthSuscripcion || car.pricePerMonth || 0), 1000);
  ...
  useEffect(() => {
    ...
    if (type === 'larga') {
      setSuscripcionType('larga');
      setFilterType('suscripcion');
    } else if (type === 'corta') {
      setSuscripcionType('corta');
      setFilterType('suscripcion');
    }
  }, [searchParams]);
  ...
  const filteredAndSortedCars = useMemo(() => {
    ...
    filtered = filtered.filter(car => {
      return car.availableForSuscripcion !== false; // Show if enabled or undefined (backward compatibility)
    });

    // Filter by price range
    filtered = filtered.filter(car => {
      const price = car.pricePerMonthSuscripcion || car.pricePerMonth || 0;
    ...
    filtered.sort((a, b) => {
      switch (sortType) {
        case 'price-asc':
          return (a.pricePerMonthSuscripcion || a.pricePerMonth || 0) - (b.pricePerMonthSuscripcion || b.pricePerMonth || 0);
        case 'price-desc':
          return (b.pricePerMonthSuscripcion || b.pricePerMonth || 0) - (a.pricePerMonthSuscripcion || a.pricePerMonth || 0);
        case 'name-asc':
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mb-8 mt-2 lg:mt-0">
        <div className="text-center">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-sm text-gray-700">{t('pages.suscripcion.reviews')}</span>
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="text-gray-900 font-medium">{t('pages.suscripcion.permanenciaLarga')}</div>
                  <div className="text-sm" style={{ color: '#000000' }}>{t('pages.suscripcion.permanenciaLargaDesc')}</div>
                </div>
              </div>
              {!(filterType === 'suscripcion' && suscripcionType === 'larga') && (
          ...
          >
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="text-gray-900 font-medium">{t('pages.suscripcion.permanenciaCorta')}</div>
                  <div className="text-sm" style={{ color: '#000000' }}>{t('pages.suscripcion.permanenciaCortaDesc')}</div>
                </div>
              </div>
              <svg width="24" height="24" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                <RightArr />
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="text-gray-900 font-medium">{t('pages.suscripcion.comprarCoche')}</div>
                  <div className="text-sm text-gray-900">{t('pages.suscripcion.comprarCocheDesc')}</div>
                </div>
              </div>
              {filterType !== 'compra' && (
                <svg width="24" height="24" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-end">
          ...
          <div className="w-full md:max-w-[calc((100%-4rem)/3)]">
            <DoubleRangeSlider
              ...
              label={t('pages.suscripcion.cuotaMensual')}
            />
          </div>
          ...
          <div className="w-full md:w-auto flex items-center justify-between md:ml-auto">
            ...
            >
              <span className="flex items-center gap-1">
                <span>↓↑</span>
                <span>{t('pages.suscripcion.ordenarPor')}</span>
              </span>
              <span className="font-bold ml-auto md:ml-0">
                {sortType === 'price-asc' || sortType === 'price-desc' ? t('pages.suscripcion.precio') : t('pages.suscripcion.nombre')}
              </span>
            </button>
          </div>
        </div>
        ...
        {filteredAndSortedCars.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 justify-items-center sm:justify-items-stretch">
            {filteredAndSortedCars.map((car) => (
              <CarDisplayCard key={car.id || car._id} car={car} basePath="/suscripcion" />
            ))}
          </div>
        ) : (
          <div className="text-center text-gray-500 py-16">
            {t('pages.suscripcion.noCoches')}
          </div>
        )}
      </section>
    </main>
