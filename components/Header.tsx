      ...
      >
          ...
          <div className="px-6 py-8">
            <div className="flex flex-col space-y-6">
              {/* Llama me!, Mi Cuenta и WhatsApp — компактный вариант только для RU */}
              <div className={`flex items-center justify-center ${locale === 'ru' ? 'gap-1.5' : 'gap-2'}`}>
                <button
                  type="button"
                  onClick={() => { setCallbackFormOpen(true); setMobileMenuOpen(false); }}
                  className={`flex items-center justify-center rounded-3xl bg-[#F4A709] font-normal text-white transition-colors hover:bg-[#E59808] whitespace-nowrap flex-1 ${
                    locale === 'ru'
                      ? 'gap-1 px-2.5 py-1.5 min-w-0 text-sm'
                      : 'gap-1.5 px-3 sm:px-4 py-2 text-base'
                  }`}
                >
                  <Phone className={`${locale === 'ru' ? 'h-4 w-4' : 'h-5 w-5'} flex-shrink-0`} />
                  <span>{t('header.llamaMe')}</span>
                </button>
                <span
                  className={`flex items-center justify-center rounded-3xl bg-[#603361] font-normal text-white whitespace-nowrap cursor-default ${
                    locale === 'ru'
                      ? 'gap-1 px-2.5 py-1.5 text-sm flex-shrink-0'
                      : 'gap-1.5 px-4 py-2 text-base flex-1'
                  }`}
                >
                  <Image
                    src="/icons/account_circle.svg"
                    alt="Account"
                    width={locale === 'ru' ? 16 : 20}
                    height={locale === 'ru' ? 16 : 20}
                    className={`${locale === 'ru' ? 'h-4 w-4' : 'h-5 w-5'} flex-shrink-0`}
                  />
                  <span>{t('header.miCuenta')}</span>
                </span>
                <a
